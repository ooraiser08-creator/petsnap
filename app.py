...
# --- [關鍵] 智慧模型輪詢函式 ---
def generate_with_fallback(prompt, image):
    # 這裡列出所有可能的模型名字，一個一個試
    candidate_models = [
        'gemini-1.5-flash-001', # 最穩定的特定版本
        'gemini-1.5-flash',     # 通用別名
        'gemini-1.5-flash-002', # 更新版
        'gemini-1.5-pro',       # 貴一點但通常都有
        'gemini-pro'            # 舊版保底
    ]
    ...
    if st.button(btn_text):
        try:
            ...
            with st.spinner(loading):
                # 呼叫我們的輪詢函式
                response = generate_with_fallback(prompt, image)
            ...
            st.error(f"系統繁忙，所有模型皆忙碌中，請稍後再試。Error: {e}")
